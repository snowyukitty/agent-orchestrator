...
is the semantic evidence region for that claim, cropped losslessly from the
same frame only after its geometry is stable. The crop exists because of arithmetic: a
...
records the capture dials, inert claim state, parent-linked crop geometry, exact
dimensions, and hashes; `npm run check`
...
1.25. `npm run promo:capture` performs two isolated runs and promotes nothing
unless their complete receipts match, so motion, timestamps, and scroll timing
cannot silently make the public evidence drift. The earlier AI-generated key
art was removed on 2026-08-29: it was
